...
//                    if (candidate_id == 0) continue;
  ...
  absl::Status SaveIndex(OutputStream &output) {
    ...
    header.set_cur_element_count(cur_element_count_);
    ...
    for (size_t i = 0; i < cur_element_count_; i++) {
      ...
      VMSDK_RETURN_IF_ERROR(
          output.SaveChunk(&size_to_serialize, sieof(size_t)));
      if (linkListSize) {
  ...
  absl::Status LoadIndex(InputStream &input, SpaceInterface<dist_t> *s,
    ...
    if (!header->ParseFromString(serialized_header.get())) {
    ...
    cur_element_count_ = header.cur_element_count();
    serialize_size_data_per_element_ = header.serialize_size_data_per_element();
    ...
    for (size_t i = 0; i < cur_element_count_; i++) {
      ...
      memcpy(&linkListSize, size_chunk, sizeof(size_t));
      ...
      if (linkListSize == 0) {
        ...
        *reinterpret_cast<char **>((*linkLists_)[i]) = nullptr;
        ...
        VMSDK_ASSIGN_OR_RETURN(auto link_list_chunk, input.LoadStringBuffer());
